...
public class ContractServiceImpl implements ContractService {
    ...
    public void add() {
        while (true) {
            ...
            String contractID = Validates.id(scanner.nextLine());
            System.out.println("Enter the deposit");
            int deposit = Integer.parseInt(Validates.rentalCost(scanner.nextLine()));
            System.out.println("Enter the fully payment");
            int fullyPayment = Integer.parseInt(Validates.rentalCost(scanner.nextLine()));
            ...
            if (co.getContractNo().toLowerCase().equals(id.toLowerCase())) {
                ...
                switch (choice) {
                    ...
                    case 2:
                        ...
                        String newBookingID = Validates.id(scanner.nextLine());
                        co.setBookingNo(newBookingID);
                        break;
                    case 3:
                        System.out.println("Enter the updated Deposit");
                        int newDeposit = Integer.parseInt(Validates.rentalCost(scanner.nextLine()));
                        co.setDeposit(newDeposit);
                        break;
                    case 4:
                        System.out.println("Enter the updated Total Payment");
                        int newPayment = Integer.parseInt(Validates.rentalCost(scanner.nextLine()));
                        co.setTotalPayment(newPayment);
                        break;
                    case 5:
                        System.out.println("Enter the updated Customer ID");
                        String newCustomerID = Validates.id(scanner.nextLine());
